...
interface Article { id: string; title: string; summary: string | null; body: string | null }
...
export default function ArticleReaderScreen() {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from('content').select('id, title, summary, body').eq('slug', slug).eq('type', 'article').eq('status', 'published').maybeSingle()
