  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          <Link href="/initiatives" className="back-link">← Initiatives</Link>
          <h1>Product Case Library</h1>
          <p className="admin-page-subtitle">{entries?.length ?? 0} entries</p>
        </div>
        <Link href="/initiatives/case-library/new" className="btn-primary">+ New entry</Link>
      </div>
            ...
            {(entries ?? []).map(entry => (
              <tr key={entry.id}>
                ...
                <td style={{ display: 'flex', gap: '0.5rem' }}>
                  <Link href={`/initiatives/case-library/${entry.id}`} className="btn-ghost" style={{ padding: '0.25rem 0.625rem', fontSize: '0.8125rem' }}>
                    Edit
                  </Link>
            ...
            {(!entries || entries.length === 0) && (
              <tr>
                <td colSpan={6} style={{ textAlign: 'center', color: '#9ca3af', padding: '2rem' }}>
                  No entries yet. <Link href="/initiatives/case-library/new" style={{ color: '#6366f1' }}>Create one →</Link>
                </td>
              </tr>
            )}
          </tbody>
